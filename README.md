...
Every new or edited text message or media caption from a non-admin group member or an external channel identity is evaluated in one Jev request using twenty spam questions plus one linkage question per recent message. For eligible human messages up to 280 characters, the bot also asks Telegram for the numeric sender's private-chat metadata and, when exposed, the sender's personal-channel title and description. Forwarded user messages use the exposed origin user ID. This metadata is cached in memory for ten minutes with a 1,000-entry bound, sent only as structured Jev input, and never logged, persisted, or added to message history. Lookup errors fail open to message-only classification. Telegram stories and profile-photo image content are not exposed to this text moderation path.
